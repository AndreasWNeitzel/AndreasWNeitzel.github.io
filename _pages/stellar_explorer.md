...
The app runs entirely client-side. The underlying evolutionary data is streamed as an HDF5 file (~152 MB) from a companion public repository on first load and decoded in the browser. No server, no installation, no file upload.
...
_First load fetches ~152 MB of data; the browser caches the file, so subsequent visits are much faster._
